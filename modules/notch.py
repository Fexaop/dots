...
from modules.osd import OSDMenu
...
class Notch(Window):
    def __init__(self, **kwargs):
        ...
        # Show after all content is added
    ...
    def close_notch(self):
        ...
        for widget in [self.launcher, self.dashboard, self.wallpapers, self.notification, self.overview, self.power, self.bluetooth, self.osd]:  # Add osd to list
            widget.remove_style_class("open")
            if widget == self.wallpapers:
                self.wallpapers.viewport.hide()
                self.wallpapers.viewport.set_property("name", None)
        for style in ["launcher", "dashboard", "wallpapers", "notification", "overview", "power", "bluetooth", "osd"]:  # Add osd to list
    ...
    def open_notch(self, widget):
        ...
        widgets = {
            ...
            "osd": self.osd  # Add OSD to widgets dictionary
        }
